...
consumer.on('message', async (message) => {
  try {
    ...
    console.log(`Pago procesado para la cuenta ${cuenta_id}, nuevo saldo: ${nuevoSaldo}`);
  } catch (err) {
    await pool.query('ROLLBACK');
    console.error('Error procesando el pago:', err.message);

    // Registrar el error en la base de datos
    const { cuenta_id } = JSON.parse(message.value);
    await registrarError(cuenta_id, err.message);
  }
});
